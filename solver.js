...
(function (root, factory) {
  ...
  function makeGraph(n, edgeList, names) {
    if (!Number.isSafeInteger(n) || n < 3) throw new Error('o modelo exige pelo menos 3 cidades inteiras');
    const edges = edgeList.map(([a, b, w], id) => ({ a, b, w, id }));
    const adj = Array.from({ length: n }, () => []);
    const seenPair = new Set();
    for (const e of edges) {
      if (e.a === e.b) throw new Error('laço em ' + e.a);
      if (![e.a, e.b].every(v => Number.isInteger(v) && v >= 0 && v < n)) throw new Error('cidade inválida na estrada ' + e.id);
      if (!Number.isSafeInteger(e.w) || e.w < 0 || e.w > Number.MAX_SAFE_INTEGER / n) throw new Error('custo deve ser inteiro não negativo e a soma deve ser segura: estrada ' + e.id);
  ...
  function boundarySet(graph, inS) {
    const B = new Set();
    for (let v = 0; v < graph.n; v++) if (inS[v] && graph.adj[v].some(e => !inS[e.to])) B.add(v);
    return B;
  }
  ...
  function solve(graph, tree, opts) {
    ...
    const maxStates = opts.maxStates ?? 5e5;      // limite por tabela, não memória total
    const maxWork = opts.maxWork ?? Infinity;
    const maxMillis = opts.maxMillis ?? Infinity;
    const start = opts.start || 0;
    const t0 = now();
    let created = 0, transitions = 0, peak = 0, width = 0, interrupted = false, reason = null;
    const nodes = [];
    function guard() {
    ...
    const members = node => node.v || members(node.l).concat(members(node.r));
    function run(node, level) {
      ...
      // Decide before computing the discarded child. A scan enumerates that region directly.
      let left = node.l, right = node.r;
      let strat = node.plan || strategy;
      const lv = members(left), rv = members(right);
      const lb = boundarySet(graph, membership(n, lv)).size, rb = boundarySet(graph, membership(n, rv)).size;
      const scanRight = lb + rv.length <= rb + lv.length;
      if (strat === 'auto') {
        const small = scanRight ? rv.length : lv.length;
        const scanWidth = Math.min(lb + rv.length, rb + lv.length);
        const estimate = signatureBound(lb) * signatureBound(rb);
        strat = small <= 4 && scanWidth <= lb + rb && estimate > productLimit ? 'varredura' : 'produto';
      }
      if (strat === 'varredura' && !scanRight && !opts.children) [left, right] = [right, left];
      const P = opts.children && node === tree ? opts.children[0] : run(left, level + 1);
      const Q = opts.children && node === tree ? opts.children[1] : strat === 'varredura'
        ? { region: membership(n, members(right)), active: [], table: new Map(), rec: { depth: 0 } }
        : run(right, level + 1);
      ...
      const rec = { kind: 'união', level, size, activeIn: active.length, boundaryOut: sw.active.length, edges: edges.length,
        ...
        work: transitions - transM, millis: now() - tMerge };
    ...
    (function validate(t) {
      ...
      (function walk(x) {
        if (x.v) { if (!x.v.length) throw new Error('folha vazia'); for (const v of x.v) { if (!Number.isInteger(v) || v < 0 || v >= n || seen[v]) throw new Error('hierarquia inválida no vértice ' + v); seen[v] = 1; } }
        else { walk(x.l); walk(x.r); }
      })(t);
      if (!opts.partial) for (let v = 0; v < n; v++) if (!seen[v]) throw new Error('vértice ' + v + ' fora da hierarquia');
    })(tree);
    ...
    const base = { mode, strategy, nodes, width, staticWidth: treeStats(graph, tree).width, work: transitions,
      depth: rootResult ? rootResult.rec.depth : nodes.reduce((w, r) => Math.max(w, r.depth), 0),
      peakStates: peak, statesCreated: created, millis: now() - t0, signatureBound: signatureBound(width),
    ...
    if (opts.partial) return Object.assign(base, { status: 'PARCIAL', packet: rootResult });
    const final = rootResult.table.get('|');
    if (!final) return Object.assign(base, { status: 'INVIAVEL', cost: null });
    const edgeIds = [];
    const pendingTrace = [final.tr];
    while (pendingTrace.length) { const tr = pendingTrace.pop(); if (!tr) continue; if ('e' in tr) { edgeIds.push(tr.e); pendingTrace.push(tr.p); } else pendingTrace.push(tr.l, tr.r); }
  ...
  function bisect(graph, opts) {
    ...
    function split(ids, rect, level) {
      ...
      cands.sort((p, q) => p.w - q.w || p.score - q.score || p.axis - q.axis || p.cut - q.cut);
